import type { HeroId, MutationId, Side } from "./types.ts";
...
export const MUTATION_RARITIES: readonly MutationRarity[] = [
  "common",
  "rare",
  "epic",
  "legendary",
...
export const MUTATION_RARITY_LABELS: Readonly<Record<MutationRarity, string>> = {
  common: "普通",
  rare: "稀有",
  epic: "史诗",
  legendary: "传说",
 ...
 */
export const MUTATION_DEFINITIONS: Readonly<Record<MutationId, MutationDefinition>> = {
  iron_wall: {
    id: "iron_wall",
    name: "堡垒",
    rarity: "common",
...
export function mutationDefinition(id: MutationId): MutationDefinition {
  return MUTATION_DEFINITIONS[id];
}

export function standardMutationsForRarity(rarity: MutationRarity): readonly MutationDefinition[] {
  return MUTATION_IDS
...
export type DestinyMutationId = `destiny:${string}`;

export interface DestinyMutationDefinition {
  id: DestinyMutationId;
  name: string;
  rarity: "legendary";
  destiny: true;
  heroBond: readonly [HeroId, HeroId];
  summary: string;
  rules: string;
  firstFormalTurnBonusMs: typeof DESTINY_FIRST_FORMAL_TURN_BONUS_MS;
}
 ...
 */
export const DESTINY_MUTATION_DEFINITIONS: readonly DestinyMutationDefinition[] = [];

export type LegendaryMutationCandidate =
  | MutationDefinition
  | DestinyMutationDefinition;

export function isDestinyUnlocked(
  definition: DestinyMutationDefinition,
...
export function legendaryMutationCandidates(
  heroes: Record<Side, HeroId>,
  destinyDefinitions: readonly DestinyMutationDefinition[] = DESTINY_MUTATION_DEFINITIONS,
): readonly LegendaryMutationCandidate[] {
  return [
