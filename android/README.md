...
> **旧实现说明（2026-09-18）：** 本文件描述当前 APK 基线。最新产品规则已要求每名玩家单局累计断线 60 秒判负、自动重连后从原节点恢复、蓝牙再战 30 秒邀请和正式对局聊天；这些内容尚未完整实现。以仓库根目录 `CONFIRMED_UI_PLAN.md` 为准。
...
网页大厅已接入该通道：蓝牙连接建立后双方先进入 60 秒英雄选择，再秘密猜拳决定先手；棋局创建后公布随机畸变、播放英雄入场，并在最多 60 秒的英雄准备中私下放置猎人陷阱。对局完整支持刺杀、强击、防护壁垒和六项畸变。背刺与裁决由房主先结算，再在两端播放同一份终结动画。断开连接会暂停当前对局；尚未实现自动断线胜负判定。
