/** Small synchronous SHA-256 implementation so room state can run in Node and WebView alike. */
const K = [
...
export function sha256Hex(value        )         {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + upper1 + choose + K[index] + words[index]) >>> 0;
...
/** Avoids an early return while comparing two same-format wire hashes. */
